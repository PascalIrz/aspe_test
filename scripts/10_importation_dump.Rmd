---
title: "Base Aspe : importation du dump"
...
Une fois la machine configurer, le package est téléchargeable depuis Github :
...
Puis, si l'on connaît les noms des tables dont on aura besoin, on les charge directement au moyen de l'argument `tables_a_extraire` en indiquant leur nom sous forme de vecteur .
...
Comme cet import est un peu long, il y a intérêt à en sauvegarder le résultat. Pour sauver tous les `dataframe` mais pas les lignes du `dump` qui ne sont plus utiles, dans le répertoire `processed_data` :

```{r, eval = FALSE}
rm(lignes_autres_tables)
save.image(file = "processed_data/tables_dump.RData")
```
