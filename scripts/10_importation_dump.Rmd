...
On importe la totalité de la base en une fois, ce qui est bien pratique. Dans la logique de l'arborescence du projet, le fichier de données - ici préalablement décompressé - est stocké dans le répertoire `raw_data`.

Cette opération prend une dizaine de minutes. Quand elle est terminée, des `Warning` sont affichés dans la console, mais pas d'erreurs. Une centaine de `dataframe` apparaissent dans l'onglet "Environnement" de RStudio.

```{r, eval = TRUE}
imp_importer_dump_sql(fichier_dump = "../raw_data/aspe_20210908.sql.gz")
```
...
```{r, eval = TRUE}
save(mesure_individuelle, file = "../processed_data/mei.RData")
rm(mesure_individuelle)
...
save.image(file = "processed_data/toutes_tables_aspe_sauf_mei.RData")
save(mei, file = "processed_data/mei.RData")
```
