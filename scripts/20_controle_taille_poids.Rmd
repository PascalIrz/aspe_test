...
tp_data <- mesure_individuelle %>% 
  filter(!is.na(mei_taille), !is.na(mei_poids), mei_mesure_reelle == "t") %>% 
    left_join(y = lot_poissons %>% select(mei_lop_id = lop_id, esp_id = lop_esp_id)) %>% 
...
gdata::keep(mesure_individuelle, passerelle, station, sure = TRUE)
```
...
Maintenant qu'on a les identifiants des stations qui nous intéressent, filtrons la passerelle à partir du vecteur  `id_stations_bzh`.

```{r}
passerelle <- passerelle %>%
  filter(sta_id %in% id_stations_bzh) # filtrage sur stations
```
