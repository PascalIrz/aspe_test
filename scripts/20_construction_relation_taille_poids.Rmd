...
Le package {rfishbase} ([Boettiger, Lang & Wainwright 2012](https://fishlab.ucdavis.edu/wp-content/uploads/sites/397/2011/01/BoettigerFishbase2012.pdf)) est une interface pour utiliser avec R 
) les données compilées dans Fishbase sur plusieurs milliers d'espèces.
...
```{r}
synthese <- synthese%>% 
  mutate(lieu = str_replace_all(lieu, " \\s*\\([^\\)]+\\)", ""),# supprimer texte entre parenthèses
         lieu = str_replace_all(lieu, "[:digit:]", ""), # supprimer les chiffres (coords, années)
         lieu = str_replace_all(lieu, "[\\-\\/]", ""), # supprimer certains caractères spéciaux
         lieu = str_squish(lieu), # supprimer les espaces qui trainent ou les fusionner
         lieu = str_replace(lieu, ",$", "")) # si le dernier caractère est une virgule, la supprimer
```
