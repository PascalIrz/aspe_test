...
Ces relations se veulent aussi générales que possible, donc on veillera à ce que des observations atypiques, voire des erreurs dans la base de données, ne viennent pas les influencer exagérément.
...
La première étape, pour éviter que des *outliers* ne perturbent les analyses, est de définir pour chaque jeu de données sur une espèce des gammes de longueurs et de poids "plausibles" en dehors desquelles les observations seront excles pour effectuer la régression.
...
- La table `mesure_individuelle` est filtrée pour supprimer les tailles et poids positifs et ne conserver que les mesures réellement effectuées.
...
tp %>% 
  ...
  DT::formatRound(columns = c('r2'), digits = 2) %>% 
...
On peut comparer avec des données de Fishbase. Il faut d'abord charger le package {rfishbase} puis utiliser sa fonction `length_weight()`.

Exemple avec la tanche, *Tinca tinca*.

```{r, eval = FALSE}
install.packages("rfishbase")
```

Chargement des données depuis Fishbase.

```{r}
fb_data <- rfishbase::length_weight("Tinca tinca") %>% 
  select(a, b, r2 = CoeffDetermination, n_ind = Number, taille_mini = LengthMin,
...
synthese <- tp %>% 
  ...
  select(names(fb_data)) %>% # mettre les colonnes dans le même ordre pour les empiler
  rbind(fb_data) %>% # empilement
  mutate(lieu = str_replace(lieu, " \\s*\\([^\\)]+\\)", "")) # supprimer texte entre parenthèses
```
...
g <- ggplot(data = synthese, aes(x = b, y = log(a, base = 10), text = lieu)) +
  ...
  geom_text(data = synthese[1,], color = "red", label = "Aspe", nudge_y = 0.07) +
  labs(y = "log10(a)")
