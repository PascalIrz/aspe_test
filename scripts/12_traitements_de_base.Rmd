...
# vecteur avec les identifiants des stations de la région (sta_id)
...
Pour éviter de surcharger notre `dataframe`, la jointure n'est pas opérée sur la totalité de la table `y`, mais uniquement sur une sélection de ses colonnes. Par exemple ci-dessous, au lieu de joindre la totalité de la table `operation_description_peche` avce ses 29 colonnes, on ne rajoute que les deux colonnes correspondant aux codes opération (`odp_ope_id` qui est renommé `ope_id` pour que la jointure se fasse automatiquement car les deux tables ont un champ en commun) et moyen de prospectionb (`odp_mop_id` qui est renommé `mop_id`).
...
densites_1col_p_sp <- densites_1col_p_sp %>%
  select(ope_id, all_of(noms_esp_ordonnes)) %>% # mise des colonnes dans l'ordre souhaité
  mutate_at(all_of(noms_esp_ordonnes), ~ifelse(is.na(.), 0, .)) # remplacement des données manquantes par 0
```
