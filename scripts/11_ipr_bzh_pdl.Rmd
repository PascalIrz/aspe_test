...
ipr_metriques <- passerelle %>% 
  ...
  filter(ope_id %in% mes_operations) %>% # filtrage sur les opérations
...
Choix d'un identifiant de station.

```{r}
mon_id_station <- ipr_metriques %>% 
  pull(sta_id) %>% 
  unique() %>% 
  .[1]
```
...
La représentation graphique va faire appel au *package* `{fmsb}` qui nécessite une mise en forme assez spécifique des données.

On veut produire le graphique pour une année = une ligne. Ici la dernière donnée disponible.

```{r}
mon_annee <- data_sta %>% 
  pull(annee) %>% 
  max()# choix d'une des lignes (= une année)
...
data_sta_1a <- data_sta %>% # tableau avec seulement la ligne de l'année sélectionnée
  filter(annee == mon_annee) 

maxi <- data_sta_1a %>% # valeur maximale pour les axes
  select(-annee) %>% 
  max() %>% 
  ceiling()
...
data_sta_1a <- rbind(bornes_sup, bornes_inf , data_sta_1a)

knitr::kable(data_sta_1a, row.names = FALSE)
```
...
radarchart(data_sta_1a %>% select(-annee),
           title = mon_titre,
           axistype = 2,
           pfcol = "grey50")
```
...
Nouveau titre sans l'année.

```{r}
mon_titre <- str_sub(mon_titre, end = -8)
```

Préparation d'une palette de couleurs pour distinguer les années avec les *packages* `{RColorBrewer}` (couleurs pleines) et `{GISTools}` (pour les transparences). 

```{r}
nb_annees <- nrow(data_sta) - 2

# Vecteurs des couleurs de ligne et de remplissage
...
```{r}
data_sta <- rbind(bornes_sup, bornes_inf , data_sta)
```


```{r, fig.height = 8, fig.width = 6.5}

radarchart(data_sta %>% select(-annee),
           axistype = 2, 
           #custom polygon
           pcol = couleurs_traits, pfcol = couleurs_remplissage, plwd = 4, plty = 1,
           #custom the grid
           cglcol = "grey", cglty = 1, axislabcol = "grey80",
           #custom labels
           vlcex = 0.8,
           title = mon_titre)
