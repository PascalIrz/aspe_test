...
```{r}
passerelle <- passerelle %>%
  mef_select_objectif(objectif = c(3, 6, 4))
```
...
ipr <- passerelle %>% 
  ...
  left_join(station %>%
              select(sta_id,
                     dept = sta_com_code_insee)) %>% 
  mutate(dept = str_sub(dept, 1, 2)) %>% 
  mef_ajouter_libelle() %>% 
  droplevels()
```
...
ipr_dept <- ipr %>%
  filter(dept == mon_dept,   # filtrage sur le département
         annee > 2009) %>%   # filtrage sur la date
...
pop_data <- ipr_metriques %>% 
  mef_ajouter_libelle() %>% 
  select(sta_id, pop_id, pop_libelle, annee, (ner:dti)) %>%
...
ggplot(data = pop_data, aes(x = annee, y = valeur)) +
