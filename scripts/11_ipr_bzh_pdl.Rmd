...
ipr_dept <- ipr %>%
  ...
  mef_filtrer_nb_mini_annees(nb_mini_annees = 5, # suppression des points avec moins de 5 années
                             var_id = "pop_id") %>% 
  select(pop_libelle,
         pop_id,
         dept,
...
Pour un groupe de stations comme le département, on utilise la même fonction en indiquant plusieurs identifiants de station dans l'argument `station_sel`.
...
On peut choisir de changer l'orientation de l'axe des ordonnées (IPR) avec l'argument `inv_y = FALSE`.
