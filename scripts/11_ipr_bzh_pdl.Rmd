...
Par défaut l'axe des ordonnées est orienté vers le bas (il est inversé pr rapport à un graphique "classique") donc plus les points sont en haut du graphique plus la qualité du milieu est élevée. On peut choisir de changer l'orientation de l'axe des ordonnées (IPR) avec l'argument `inv_y = FALSE`.
...
Pour sélectionner un groupe de stations, on utilise la même fonction en indiquant plusieurs identifiants de station dans l'argument `station_sel`. Sinon, elles sont toutes conservées. 
